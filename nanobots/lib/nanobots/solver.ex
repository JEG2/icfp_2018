defmodule Nanobots.Solver do
  ...
  def solve(%__MODULE__{state: state} = solver) do
    commands = generate_timestep(solver)
    new_state = State.apply(state, commands)
    if State.end?(new_state) do
      IO.puts new_state.energy
    else
      solve(%__MODULE__{solver | state: new_state})
    end
  end

  def generate_timestep(%__MODULE__{state: state, strategy: strategy}) do
    strategy.move(state)
  end
end
